...
from core.feature_xlnet import *
...
def get_train_test_bert():
    ...
    data = get_feature_xlnet(SEQ_LEN)
...
def train_base():
    ...
    with timed_bolck(f'Prepare train data#{BATCH_SIZE}, LR:{LR}'):
        ...
        logger.info(f'Start to train base on checkpoint:{checkpoint_path}')
        ...
        output = keras.layers.Dense(units=num_classes, activation='softmax', name='Softmax')(dropout)
    ...
    with timed_bolck(f'Training#{fold}'):
        ...
        trunc_len_tran = 64*(len(train_idx)//64)
        trunc_len_val = 64 * (len(test_idx) // 64)

        train_idx = train_idx[:trunc_len_tran]
        test_idx = test_idx[:trunc_len_val]
        ...
        input3  =  np.zeros_like(input1.iloc[:, :1])

        val1 = val_x.loc[:, input1_col]
        val2 = np.zeros_like(val1)
        val3 = np.zeros_like(val1.iloc[:,:1])
        ...
        es = EarlyStopping(monitor='val_acc',patience=2, verbose=1)
        with tf.keras.utils.custom_object_scope(get_custom_objects()):
            his = model.fit([input1, input2, input3], train_y,
                            ...
                            callbacks=[Cal_acc( val_x, y.iloc[test_idx] ), es]
...
class Cal_acc(Callback):
    ...
    def cal_acc(self):
        ...
        input1 = self.val_x.loc[:,input1_col]  # .astype(np.float32)
        input2 = np.zeros_like(input1)  # .astype(np.int8)
        input3 = np.zeros_like(input1.iloc[:, :1])

        val = self.model.predict([input1, input2, input3])

        label2id, id2label = get_label_id()
        val = pd.DataFrame(val, columns=label2id.keys(), index=input1.index)
    ...
    def cal_acc_ex(self):
        ...
        for type_ in tqdm(check_type_list,desc='cal_acc_ex'):
            ...
            input1 = tmp_val.loc[:, input1_col]  # .astype(np.float32)
            input2 = np.zeros_like(input1)  # .astype(np.int8)
            input3 = np.zeros_like(input1.iloc[:, :1])
            logger.info(f'{input1.shape},{input2.shape},{input3.shape}')
            logger.info(input1[:3])
            # logger.info(input2[:3])
            # logger.info(input3[:3])
            val = self.model.predict([input1,input2,input3])
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if self.trial > 0:
            ...
            if ( round(total,4) > round(self.threshold,4)
                 and (epoch>=3 or self.threshold > 0 or total>0.83 )
                 and total > max(self.max_score, 0.83)
                ) :
                ...
                min_len_ratio = get_args().min_len_ratio
                oof_file = f'./output/stacking/{oof_prefix}_{self.fold}_{total:7.6f}_{len_raw_val}_{len(val):05}_b{get_args().max_bin}_e{epoch}_{self.batch_id}_m{min_len_ratio:2.1f}_L{SEQ_LEN:03}_XL.h5'
    ...
    def gen_sub(self, model , info='bert_' , partition_len = 5000):
        ...
        for sn in tqdm(range(1+ len(test)//partition_len), desc=f'{info}:sub:total:{len(test)},partition_len:{partition_len}'):
            ...
            #print('\nbegin tmp\n', tmp.iloc[:3,:3].head())
            input1 = tmp.loc[:,input1_col]
            input2 = np.zeros_like(input1)  # .astype(np.int8)
            input3 = np.zeros_like(input1.iloc[:, :1])
            res = model.predict([ input1, input2, input3])
